...
{synopt:} lddtest performs equivalence testing on logarithmic density discontinuities of running variables at the cutoff
...
{col 5}{it:epsilon}{col 21}Largest ratio between running variable density estimates on each side of the cutoff that would be considered 'practically equal to 1'. Required, must be a real number > 1.
{space 4}{hline}
...
{space 4}{hline}
{col 5}{it:b}{col 21}Width of histograms used to initialize the DCdensity estimation. If not specified, bin width defaults to that specified in Section 3.2 of McCrary (2008).
{col 5}{it:h}{col 21}Width of symmetric bandwidth used to compute the DCdensity estimation. If not specified, bandwidth defaults to that specified in Section 3.2 of McCrary (2008).
{col 5}{it:alpha}{col 21}Significance level. Defaults to 0.05. If specified, must be a real number strictly between 0 and 0.5.
{col 5}{it:at}{col 21}Name of variable with values at which to compute initial density estimates. If not specified, defaults to the equi-spaced grid specified in Section 3.1 of McCrary (2008). If specified, must be a string.
